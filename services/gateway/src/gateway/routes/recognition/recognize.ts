...
export function createRecognizeRoute() {
    return new Hono().post('/recognize', async c => {
        ...
        try {
            ...
            const alignmentMode = (formData.get('alignmentMode') as string | undefined) || 'neural'

            if (!image) {
                return c.json({ error: 'image is required' }, 400)
            }

            if (alignmentMode !== 'classic' && alignmentMode !== 'neural') {
                return c.json({
                    error: `invalid alignment mode: ${alignmentMode}. allowed: classic, neural`,
                }, 400)
            }
            ...
            const result = await services.ocr.uploadImage(image, {
                ...
                sourceReference,
                acceptedQrFormats,
                alignmentMode: alignmentMode as 'classic' | 'neural',
            })
